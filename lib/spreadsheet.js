...
module.exports.creareWorksheet = function (opts, callback) {
    if (!callback)
        callback = opts.callback;
    if (!callback)
        throw "Missing callback";
    ...
    if ( !opts.worksheetName)
        return callback("Missing 'worksheetName'");
    ...
    auth(opts, function (err, token) {
        ...
        spreadsheet.log('Logged into Google'.green);
        spreadsheet.setToken(token);        
        spreadsheet.createSheet(opts.worksheetName , opts.rows || 100, opts.col || 5, function (err) {
            if (!err) {
                spreadsheet.init(callback);
            } else {
                return callback("worksheet '" + opts.worksheetName + "' not created: " + err);                
            }
        });        
    });
};
...
Spreadsheet.prototype.createSheet = function (worksheetName, rows, cols, callback) {
	request({
		url: this.worksheetUrl(),
		method: 'POST' ,
		body: createSheetXML(worksheetName , rows , cols),
		headers: this.postAuthHeaders
    }, function (error, response, body) {
        if (error)
            callback(error);
        else if (response.statusCode != 201)
            callback(response.body); //reason into response.body if sheet not created.
        else
            callback(null); //sheet created
	});
};
